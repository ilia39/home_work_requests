...
def yandex_disc_put(file, fname_disc):
    ...
    answer_data = requests.get(upload_url_request, headers=headers, params=params)
...
def get_stackoverflow_questions():
    ...
    date_timestamp_str = datetime.datetime.strptime(date_str, '%Y-%m-%d').strftime("%s")
    date_deep2 = datetime.datetime.utcnow().date() - datetime.timedelta(2)
    date_deep2_str = date_deep2.strftime('%Y-%m-%d')
    date_deep2_timestamp = datetime.datetime.strptime(date_deep2_str, '%Y-%m-%d').strftime("%s")
    params_base = {
